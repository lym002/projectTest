...
import com.jzh.data.FuiouOnlineBanking12Req;
...
public class FuiouConfig {
	...
	public final static String REGURL;//个人用户自助开户注册（直连）
	public final static String WEBREGURL;//个人用户自助开户注册（网页版）
	public final static String QRECHARGEURL;//商户P2P网站免登录快速充值（网页版）
	...
	static { ////注意 properties 里统一小写
		...
		WEBREGURL = ConfigReader.getConfig("jzh_url") + ConfigReader.getConfig("webregurl");
		QRECHARGEURL = ConfigReader.getConfig("jzh_url") + ConfigReader.getConfig("jzh_url") + ConfigReader.getConfig("500001url");
...
//		data.setMchnt_txn_ssn(params.get("mchnt_txn_ssn").toString());
	 ...
	 * @return
	 */
	public static String rechargeFist(Map<String,Object> map){
...
//		data.setVer(FuiouConfig.VER);
	 ...
	 * @return
	 */
	public static String onlineBankingRecharge12Data(Map<String,String> param){
		FuiouOnlineBanking12Req data = new FuiouOnlineBanking12Req();
		String amt = yuanToCent(param.get("amt"));
		data.setMchnt_cd(FuiouConfig.MCHNT_CD);
		data.setMchnt_txn_ssn(param.get("mchnt_txn_ssn"));
		data.setAmt(amt);
		data.setIss_ins_cd(param.get("iss_ins_cd"));
		data.setOrder_pay_type(param.get("order_pay_type"));
		
		data.setLogin_id(param.get("login_id"));
		data.setPage_notify_url(FuiouConfig.OPENACCOUNT_PAGE_NOTIFY_URL);
		data.setBack_notify_url(FuiouConfig.OPENACCOUNT_BACK_NOTIFY_URL);
		//记录日志
		SysFuiouNoticeLog sysFuiouNoticeLog = new SysFuiouNoticeLog(data);
		sysFuiouNoticeLog.setIcd(BRECHARGE12);
		sysFuiouNoticeLog.setAmt(param.get("amt"));
		sysFuiouNoticeLog.setUser_id(data.getLogin_id());
		sysFuiouNoticeLog.setIcd_name("网银充值（页面）");
		fuiouConfig.sysFuiouNoticeLogService.insert(sysFuiouNoticeLog);
		return sysFuiouNoticeLog.getReq_message();
...
//		data.setCertif_tp("0");//证件类型
		...
		//记录日志
		SysFuiouNoticeLog sysFuiouNoticeLog = new SysFuiouNoticeLog(data);
		sysFuiouNoticeLog.setIcd(WEBREG);
		sysFuiouNoticeLog.setUser_id(data.getMobile_no());
		sysFuiouNoticeLog.setIcd_name("个人开户（页面）");
		fuiouConfig.sysFuiouNoticeLogService.insert(sysFuiouNoticeLog);
	 ...
	 * 支付密码修改(页面)
	 * @param user_id 存管id
	 * @param busi_tp	业务类型;//1:重置登录密码,2:修改登录密码,3:支付密码重置
	 * @param id uid
	 * @return
	 */
	public static String resetPassword(Map<String,Object> map){
		...
		//记录日志
		SysFuiouNoticeLog sysFuiouNoticeLog = new SysFuiouNoticeLog(data);
		sysFuiouNoticeLog.setIcd(RESETPASSWORD);
		sysFuiouNoticeLog.setUser_id(data.getLogin_id());
		...
		obj.getJSONObject("message").put("back_url", OPENACCOUNT_PAGE_NOTIFY_URL);
		result = obj.toString();
		return result;
...
//		data.setRem(map.get("rem"));//备注 o
		...
		sysFuiouNoticeLog.setIcd_name("开户直连pc");
		sysFuiouNoticeLog.setUser_id(data.getMobile_no());
		sysFuiouNoticeLog.setChannel(0);
		fuiouConfig.sysFuiouNoticeLogService.insert(sysFuiouNoticeLog);
		try {
