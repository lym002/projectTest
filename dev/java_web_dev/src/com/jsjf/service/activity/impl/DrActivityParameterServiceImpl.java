...
public class DrActivityParameterServiceImpl implements DrActivityParameterService {
	...
	private static transient Logger log = Logger.getLogger(DrActivityParameterServiceImpl.class);
	...
	@Override
	public DrCouponsIssuedRules getcouponsIssuedRules(Map<String, Object> map)
			throws Exception {
		return drCouponsIssuedRulesDAO.getCouponsIssuedRules(map);
	}
	...
	public BaseResult getRandomCouponByProductId(DrMember m, int pid) {
		...
		try {
			//是否投资普标
			if(drProductInvestDAO.selectInvestCountExcludeNewHand(m.getUid()) <= 0){
			...
			DrActivityParameter dap = mapdata.get(rate);//可能为空,待测
			DrMemberFavourable dmf = new DrMemberFavourable(dap.getId(), m.getUid(), 2, dap.getCode(), dap.getName(), dap.getAmount(),
	...
	public List<Map<String, Object>> getActivityParameterByRules() {
		...
		List<Map<String,Object>> couponIds = new ArrayList<Map<String,Object>>();
		DrCouponsIssuedRules issuedRules = drCouponsIssuedRulesDAO.getCouponsIssuedRules(map);
		if(Utils.isObjectNotEmpty(issuedRules)){
			String[] couponId = issuedRules.getCoupons().split(",");
			map.clear();
			map.put("couponIds", couponId);
			couponIds = drActivityParameterDAO.selectActivityParameterByMap(map);
		}
		return couponIds;
	}
