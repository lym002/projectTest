...
public class BypCommodityDetailServiceImpl implements BypCommodityDetailService {
            ...
            if (nowDate.after(Utils.parseDate(doubleDanStartDate,
                ...
                if (lockFlag) {
                    ...
                    if (dm.getUser_integral().compareTo(bcd.getNeedPoints()) == 0 || dm.getUser_integral().compareTo(bcd.getNeedPoints()) == 1) {
                        ...
                        DrMemberMsg msg = new DrMemberMsg(uid, 0, 1, "恭喜您成功领取" + bypCommodityDetail.getPrizeName() + "奖励！", new Date(), 0, 0, "恭喜您在“捕鱼达人活动-4月有豪礼”成功领取" + bypCommodityDetail.getPrizeName() + "，奖励于活动结束后3-15个工作日统一发放，别忘了填写正确的邮寄地址，有疑问请联系客服：400-820-4684，奖励可以多次领取！");
    ...
    public BaseResult updateIntegralConvertGiftByUidAndPid(Integer uid, String pid, Integer number) {
        ...
        BaseResult br = new BaseResult();
        Map<String, Object> map = new HashMap<String, Object>();
        JsMemberInfo selectMemberInfoByUid = null;
            ...
            if (dm.getUserIntegralUse().compareTo(bcd.getNeedPoints().multiply(new BigDecimal(number))) >=0 ) {
                for (int i = 0; i < number; i++) {
                    ...
                    if (dm.getUserIntegralUse().compareTo(bcd.getNeedPoints()) == 0 || dm.getUserIntegralUse().compareTo(bcd.getNeedPoints()) == 1) {
                        ...
                        // 查询客户是否有地址
                        if (selectMemberInfoByUid == null) {
                            selectMemberInfoByUid = jsMemberInfoDAO
                                    .selectMemberInfoByUid(uid);
                        }
                        if (Utils.isObjectEmpty(selectMemberInfoByUid)) {
                            // 插入客户兑换记录
                        ...
                        //根据UID和创建查询用户礼品
//							BypCommodityDetail bypCommodityDetail=  bypCommodityDetailDAO.selectConvertGiftByUid(uid);
//							DrMemberMsg msg = new DrMemberMsg(uid, 0, 1, "恭喜您成功兑换"+bypCommodityDetail.getPrizeName()+"！", new Date(), 0,0, "恭喜成功兑换"+bypCommodityDetail.getPrizeName()+"，积分商品在3-15个工作日统一发放，别忘了填写正确的邮寄地址，有疑问请联系客服：400-820-4684，物品可以多次兑换哦！");
//							drMemberMsgDAO.insertDrMemberMsg(msg);
                        br.setSuccess(true);
                    } else {
                        br.setErrorCode("1003");
...
//					}
                        ...
                        //根据UID和创建查询用户礼品
//						BypCommodityDetail bypCommodityDetail = bypCommodityDetailDAO.selectConvertGiftByUid(uid);
//						DrMemberMsg msg = new DrMemberMsg(uid, 0, 1, "恭喜您成功兑换" + bypCommodityDetail.getPrizeName() + "！", new Date(), 0, 0, "恭喜您在成功兑换" + bypCommodityDetail.getPrizeName() + "，有疑问请联系客服：400-820-4684，红包可以多次兑换哦！");
//						drMemberMsgDAO.insertDrMemberMsg(msg);
                        br.setSuccess(true);
                    } else {
                        br.setErrorCode("1003");
