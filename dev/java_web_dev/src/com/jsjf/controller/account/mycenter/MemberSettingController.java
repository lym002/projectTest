...
//				}
	...
	public String updateLoginPassWord(@RequestBody Map<String, Object> param, HttpServletRequest req){
		...
		//原密码不对
		if(StringUtils.isBlank(oldPwd) || !member.getPassWord().equals(SecurityUtils.MD5(oldPwd))){
			br.setSuccess(false);
        	br.setErrorCode("1001");
        	return JSON.toJSONString(br);
		...
		member.setPassWord(SecurityUtils.MD5(newPwd));
		try {
			drMemberService.update(member);
			br.setSuccess(true);
			req.getSession().setAttribute(ConfigUtil.FRONT_LOGIN_USER, loginMember);
			redisClientTemplate.del(ConfigUtil.PWD_ERROR+ member.getMobilephone());
			// 发送手机短信
