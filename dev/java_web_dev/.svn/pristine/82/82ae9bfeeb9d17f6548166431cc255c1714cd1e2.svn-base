...
//	public static String certPath = "/usr/local/tomcat/config/jyt-key/jytpayserver.cer";//证书存放路径
	public static String pfxPath = "D:\\workspace\\Workspace_web\\merchantTest.pfx";
	public static String certPath = "D:\\workspace\\Workspace_web\\jytpayserver.cer";//证书存放路径
	private static String certPasswd = "password";// 证书密码
	
	public static String MERCHANT_ID = "290067600001";	//测试环境测试认证商户号
	public static String FOUR_ELEMENTS ="http://test1.jytpay.com:20080/JytAuth/tranCenter/authReq.do";	//四要素测试地址
	...
	public static String WY_SERVER_URL = "http://220.248.70.90:30080/JytNetpay/payment.do";	//测试网银充值URL
//	测试环境
//	public static String WY_BACK_Url = "http://192.168.1.246/rechargeSuccess";	//测试网银充值同步URL
//	public static String WY_NOTIFY_Url = "http://192.168.1.246/recharge/receiveNotifyJYTWY.do";	//测试网银充值异步URL
	//本地
	public static String WY_BACK_Url = "http://127.0.0.1/rechargeSuccess";	//测试网银充值同步URL
	public static String WY_NOTIFY_Url = "http://127.0.0.1/recharge/receiveNotifyJYTWY.do";	//测试网银充值异步URL


	//四要素类型	
	public static String FOUR_ELEMENTS_TRAN_CODE = "TR4003";
	//代付代码
	 ...
	 * 四要素验证
	 */
	public JYTResultData getFourElements(JYTFourElementsSendDate sendData) throws Exception{
		String xml = getTR4003Xml(sendData);
		String mac = signMsg(xml);
        String respXml = sendFourElementsMsg(xml, mac);
        JYTResultData paymentDataBean = payResultMsg(respXml);
        return paymentDataBean;
	}
	/**
	 * 发送四要素验证信息
	 */
	public String sendFourElementsMsg(String xml,String sign) throws Exception{
		log.info("上送报文："+xml);
//		log.info("上送签名："+sign);
		...
		String res = HttpClient431Util.doPost(paramMap, FOUR_ELEMENTS);
...
//		log.info("返回报文签名:"+respSign);
	...
	public JYTResultData payResultMsg(String respMsg) throws Exception{
		...
		if(Utils.isObjectNotEmpty(packageBody)){
			...
			Node bank_name = packageBody.selectSingleNode("bank_name");
			...
			//四要素返回银行名称
			resultData.setBank_name(bank_name == null ? "" : bank_name.getText());
		}
	 ...
	 * @return
	 */
	public String getTR4003Xml(JYTFourElementsSendDate sendData){
		
		StringBuffer xml = new StringBuffer();
		xml.append(getMsgHeadXml(sendData.getTran_code(),sendData.getTran_flowid()));
		xml.append("<body>");
		xml.append("<bank_card_no>").append(sendData.getBank_card_no()).append("</bank_card_no>");
		xml.append("<id_num>").append(sendData.getId_num()).append("</id_num>");
		xml.append("<id_name>").append(sendData.getId_name()).append("</id_name>");
		xml.append("<terminal_type>").append(sendData.getTerminal_type()).append("</terminal_type>");
		xml.append("<phone_no>").append(sendData.getPhone_no()).append("</phone_no>");
		xml.append("<bank_card_type>").append(sendData.getBank_card_type()).append("</bank_card_type><bank_code></bank_code>");
	...
	public String getSc0001Xml(JYTSendData sendData){
		...
		if(Utils.strIsNull(sendData.getOri_tran_flowid())){
			...
			if(!Utils.strIsNull(sendData.getCert_type())){
				xml.append("<cert_type>").append(sendData.getCert_type()).append("</cert_type>");
			}
			if(!Utils.strIsNull(sendData.getCert_no())){
				xml.append("<cert_no>").append(sendData.getCert_no()).append("</cert_no>");
			}
		}else{
