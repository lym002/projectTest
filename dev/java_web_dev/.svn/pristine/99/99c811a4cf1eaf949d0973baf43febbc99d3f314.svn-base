...
public class RedisClientTemplate {
	...
	public String getProperties(String key) throws IOException{
		...
		Properties pro = (Properties)SerializeUtil.unserialize(get("properties".getBytes()));
		if(Utils.isObjectEmpty(pro)){
			...
			pro.load(Thread.currentThread().getContextClassLoader().getResourceAsStream("resources/mobileAndMailSet.properties"));
			setex("properties".getBytes(),7200,SerializeUtil.serialize(pro));
			pro = (Properties)SerializeUtil.unserialize(get("properties".getBytes()));
			if(key == null)
				return "";
			else
