...
//				map.put("status", 1);
				...
				afid = list.get(0).getId();
				activity = list.get(0);
			}else{
				activity = jsActivityFriendDAO.selectByPrimaryKey(afid);
			}*/
			//好友总数
...
//			param.put("afid",afid);
			...
			if(!Utils.isBlank(pageOn)){
				pageInfo.setPageOn(pageOn);
			}
			if(!Utils.isBlank(pageSize)){
				pageInfo.setPageSize(pageSize);	
			}
			...
			pageInfo.setTotal(total);
			pageInfo.setRows(friendList);
			
			map.clear();
//			map.put("afid",afid);
			map.put("friendTotal", friendTotal);
...
//			map.put("status", 1);
			...
			if(!Utils.isEmptyList(list)){			
				...
				map.put("banner", list.get(0).getAppPutImg());//banner
				map.put("jumpUrl", list.get(0).getAppPutUrl());//banner链接地址
