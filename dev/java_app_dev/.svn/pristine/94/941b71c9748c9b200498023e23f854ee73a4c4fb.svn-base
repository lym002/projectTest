...
import com.jsjf.service.vip.impl.VipActivityParameterServiceImpl;
...
public class MemberVipController {
    ...
    public String index(HttpServletRequest req, Integer uid){
        ...
        br = memberVipInfoService.memberVipInfo(uid);
        return JSON.toJSONString(br);
    }
    ...
    @ResponseBody
    public String monthPacket(HttpServletRequest req,Integer uid){
        //会员中心页面基础数据
        BaseResult br = new BaseResult();
        if (Utils.isObjectEmpty(uid)) {
            br.setErrorMsg("未登录");
        ...
        try {
            ...
            br = vipActivityParameterService.queryMonthPacket(memberVipInfo.getVipLevel(), uid);
        }catch (Exception e){
    ...
    @ResponseBody
    public String getMonthPacket(HttpServletRequest req, Integer uid, Integer fid){
        //会员中心查询用户月度礼包
        BaseResult br = new BaseResult();
        if (Utils.isObjectEmpty(uid)) {
            br.setErrorMsg("未登录");
        ...
        boolean flag = vipActivityParameterService.queryMonthPacketIsGet(uid);
        if (flag){
        ...
        if (drActivityParameter != null) {
            try {
                ...
                if (vipEquities != null) {
                    vipEquitiesMemberService.insertSelective(new VipEquitiesMember(null, uid, drMember.getMobilephone(), info.getVipLevel(), vipEquities.getId(), new Date()));
                }
                br.setSuccess(true);
                br.setMsg("领取成功！");
            } catch (Exception e) {
