...
<body>
	...
	<div id="drChannelInfoOrderTools" style="padding:5px;height:750">
	  	<form id="drChannelInfoOrderForm">
	    	...
	    	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-redo" onclick="drFistChannelInfoOrderDown()">首投导出</a>	
	    </form>
	</div>
<script type="text/javascript">
	...
	$('#searchDrChannelInfoOrder').click(function(){
		...
		if(isAuth == 1){
			$('#drChannelInfoOrderList').datagrid({
	 			url: "../channel/drChannelInfoOrderList.do",
				queryParams: {
	...
	$('#searchFistDrChannelInfoOrder').click(function(){
		...
		if(isAuth == 1){
			$('#drChannelInfoOrderList').datagrid({
				url:"../channel/drFistChannelInfoOrderList.do",
				queryParams: {
					startDate: $('#ssearchDrChannelInfoOrderStartDate').datebox('getValue'),
					endDate: $('#searchDrChannelInfoOrderEndDate').datebox('getValue'),
					startInvestDate:$('#ssearchDrChannelInfoOrderStartInvestDate').datebox('getValue'),
	...
	$('#searchAgainDrChannelInfoOrder').click(function(){
		...
		if(isAuth == 1){
			$('#drChannelInfoOrderList').datagrid({
				url:"../channel/drAgainChannelInfoOrderList.do",
				queryParams: {
					startDate: $('#ssearchDrChannelInfoOrderStartDate').datebox('getValue'),
					endDate: $('#searchDrChannelInfoOrderEndDate').datebox('getValue'),
					startInvestDate:$('#ssearchDrChannelInfoOrderStartInvestDate').datebox('getValue'),
	...
	function drChannelInfoOrderDown(){
		...
		location.href="${apppath}/channel/exportChannelOrderRecord.do?mobilephone="+$('#searchDrChannelInfoOrderPhone').val()
						...
						"&code="+$('#searchDrChannelInfoOrderCode').combobox('getValue')+
						"&deadline="+$('#deadline').combobox('getValue')+
						"&isbank="+$('#isbank').combobox('getValue')+"&investId="+$('#investId').val().trim()+
						"&repayType="+$('#repayType').combobox('getValue');
		}
		if(isAuth == 0){
			//非渠道用户
	...
	//导出首投按钮
	function drFistChannelInfoOrderDown(){
		var isAuth = $('#isAuth').val();
		if(isAuth == 1){
			location.href="${apppath}/channel/exportFistChannelOrderRecord.do?mobilephone="+$('#searchDrChannelInfoOrderPhone').val()
			+"&startDate="+$('#ssearchDrChannelInfoOrderStartDate').datebox('getValue')+
			"&endDate="+$('#searchDrChannelInfoOrderEndDate').datebox('getValue')+
			"&startInvestDate="+$('#ssearchDrChannelInfoOrderStartInvestDate').datebox('getValue')+
			"&endInvestDate="+$('#searchDrChannelInfoOrderEndInvestDate').datebox('getValue')+
