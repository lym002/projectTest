    ...
    public void insertFriendMemberActivityAmount() {
        ...
        for (int i = 0; i < loopNum; i++) {
            ...
            for (DrMemberFunds dto : reconciliationList) {
                ...
                balanceMap.put(dto.getMobilePhone(), dto.getFuiou_balance().add(dto.getFuiou_freeze()));
            }
            ...
            if (balanceMap.get(mobilePhone).compareTo(fuiouBalance) != 0){
                log.warn("账户余额错误：>>>>>>>>> 手机号：" + mobilePhone + ">>>>>>>>> 富有返回余额：" + fuiouBalance + " >>>>>>>>> 本地数据余额：" + balanceMap.get(mobilePhone));
            }
            balanceMap.clear();
            requestMap.clear();
            jsonObject.clear();
