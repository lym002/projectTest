...
public class RedisClientTemplate {
     ...
     * @param timeout
     * @param unit
     * @param auto 是否设置锁的失效时间，为false则需要手动释放锁
     * @param value redis值
     * @return
     */
    public boolean tryLock(String key, long timeout, TimeUnit unit, boolean auto,String value) {
        ShardedJedis shardedJedis = redisDataSource.getRedisClient();
        boolean broken = false;
        try {
            long nano = System.nanoTime();
            do {
                Long i = shardedJedis.setnx(key, value);
                if (i == 1) {
                    if(auto){
                        shardedJedis.expire(key, 30);
                    }
                    return Boolean.TRUE;
                }
                if (timeout == 0) {
    ...
    public void lock(String key) {
        ...
        //以用户ｉｄ为ｋｅｙ，不同用户调用同一个方法不会加锁
        boolean broken = false;
        try {
            if (shardedJedis != null) {
                Long lock = shardedJedis.setnx(key, time + "");
                while (lock == 0) {
                    TimeUnit.MILLISECONDS.sleep(50);
                    lock = shardedJedis.setnx(key, time + "");
                }
                //设置超时时间
                shardedJedis.expire(key, 3);
            }
            //自己的业务逻辑处理
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @return 
     */  
    public boolean tryLock(String key, long timeout, TimeUnit unit, boolean auto) {  
        ShardedJedis shardedJedis = redisDataSource.getRedisClient(); 
        boolean broken = false;
        try {  
            long nano = System.nanoTime();  
	...
	public String getProperties(String key) throws IOException{
		...
		Properties pro = (Properties)SerializeUtil.unserialize(get("sys_properties".getBytes()));
		if(Utils.isObjectEmpty(pro)){
			pro = new Properties();
			pro.load(Thread.currentThread().getContextClassLoader().getResourceAsStream("resources/informContent.properties"));
			SerializeUtil.serialize(pro);
			setex("sys_properties".getBytes(),7200,SerializeUtil.serialize(pro));
			pro = (Properties)SerializeUtil.unserialize(get("sys_properties".getBytes()));
			if(key == null)
				return "";
			else
     ...
     * @return
     */
    public Set<String> getSet(String prefix)throws IOException {
    	Properties pro = (Properties)SerializeUtil.unserialize(get("sysproperties".getBytes()));
    	pro = new Properties();
    	pro.load(Thread.currentThread().getContextClassLoader().getResourceAsStream("resources/informContent.properties"));
		if (pro == null || prefix == null) {
