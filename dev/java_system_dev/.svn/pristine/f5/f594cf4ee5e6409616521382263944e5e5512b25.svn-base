...
<div id="${tabid }" style="width:100%;">
	<div title="产品信息" class="easyui-panel" style="width:100%;height:auto;padding:10px;margin-bottom: 10px" data-options="collapsible:true,region:'center'">
		<table id="updateProductInfo" align="center" cellspacing="1" cellpadding="1" style="width:auto;">
			...
			<tr>
				...
				<td colspan="2">
					<input name="deadline" value="${drProductInfo.deadline}" type="text" class="easyui-numberbox" disabled="disabled"/>天
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="2">
					<input name="maxAmount" value="${drProductInfo.maxAmount}" type="text" class="easyui-numberbox" disabled="disabled"/>天
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td align="left">
					...
					<td align="left">
						<select name="prizeId" style="width: 172px" class="easyui-combobox" id="prizeId" disabled="disabled">
			...
			<c:forEach items="${drClaimsPic}" var="v" varStatus="i">
				...
				<td>
					<input type="checkbox" name="drClaimsPic[${i.index}].isShow" value="1" <c:if test="${v.isShow == 1}"> checked="checked" </c:if> disabled="disabled"/>
	   	   		...
	   	   		<td>
	   	   			<input name="drClaimsPic[${i.index}].showSort" value="${v.showSort}" type='text' class='easyui-numberbox' data-options="multiline:true" disabled="disabled"/> 
	   	   		</td>
   	   		</tr>
			</c:forEach>
		</table>
	...
	<div title="产品图片" class="easyui-panel" style="width:100%;height:auto;padding:10px;margin-bottom: 10px"data-options="collapsible:true,region:'center'">
		<table id="picTable" style="width:auto;" align="center" >
			<tr>
				<td>
	                <div id="imgs" class="imgs" style="overflow-x: auto;width: 820px;">
	...
	<div title="<center>产品审核</center>" class="easyui-panel" style="width:100%;height:auto;padding:10px;margin-bottom: 10px" data-options="collapsible:true,region:'center'">
		...
		<input id="drProductAuditId" type="hidden" value="${drProductInfo.id }" name="id"/>
		<table style="width:auto;" align="center" >
	   	   	<tr>
				<td align="left">审核意见：</td>
				<td colspan="3">
					<input id="DrProductAuditOpinion" class="easyui-textbox" name="opinion" style="height:123px;width: 500px;" data-options="multiline:true,required:true,validType:'maxLength[255]'"/>
				</td>
	   	   	</tr>
	   	   	<tr>
	   	   		<td></td>
	   	   		<td align="center"><a href="javascript:void(0)" class="easyui-linkbutton" data-value="1" onclick="addDrProductInfoAudit(this);">通过</a></td>
	   	   		<td align="center"><a href="javascript:void(0)" class="easyui-linkbutton" data-value="2" onclick="addDrProductInfoAudit(this);">驳回</a></td>
	   	   		<td align="center"><a href="javascript:void(0)" class="easyui-linkbutton" data-value="3" onclick="addDrProductInfoAudit(this);">作废</a></td>
	   	   	</tr>
		</table>
		</form>
	</div>
...
<script type="text/javascript">
	...
	//审核
	function addDrProductInfoAudit(obj){   	
		var url = "../product/toAuditDrProductInfoList.do";
		var validate = $("#auditDrProductInfoForm").form("validate");
		if(!validate){
			return false;
		}
		...
		$.messager.confirm("审核提示", title, function(ensure){
			if(ensure){
		   		$.ajax({
		          	url: "${apppath}/product/addDrAuditInfo.do?opinion="+$("#DrProductAuditOpinion").val()+"&pid="+$("#drProductAuditId").val()+"&status="+status,
		            type: 'POST',
		      		success:function(result){
						if(result.success){
							$.messager.alert("提示信息",result.msg,"",function(){
								var currTab = parent.$('#main-center').tabs('getTab', "产品审核");
								if(currTab != null){
								...
								parent.$('#main-center').tabs('close','产品管理-审核');
							});
							$("#auditDrProductInfoForm").resetForm(); // 提交后重置表单
						}else{
