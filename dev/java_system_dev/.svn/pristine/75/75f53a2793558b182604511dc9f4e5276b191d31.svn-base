...
public class ActivityTask {
     ...
     * 5880活动定时任务
     */
    @Scheduled(cron = "58 59 23 * * ?") //凌晨23.59执行统计当日土豪榜
    public void setEveryoneJdCard() {
        try {
            Map<String,Object> map=new HashMap<>();
            String thisDate = Utils.format(new Date(), "yyyy-MM-dd");
            String dayBegin = Utils.format(Utils.setDayBegin(thisDate), "yyyy-MM-dd HH:mm:ss");
            String dayEnd = Utils.format(Utils.setDayEnd(thisDate), "yyyy-MM-dd HH:mm:ss");
            ...
            log.info("开始执行5880活动定时任务"+dayBegin+"-->"+dayEnd);
