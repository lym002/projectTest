...
<body>
	<table id="drProductLoanList" title="存管新手标回款管理" style="height:99%;width:99.9%">
		<thead>
	    <tr>
	        ...
	        <th data-options="field:'loanName'" width="15%" >借款方</th>
	...
	<div id="drProductLoanTools" style="padding:5px;height:750">
	  	<form id="drProductLoanForm" target="_blank" method="post">
	  		...
	  		产品状态:<select name="status" style="width: 150px" class="easyui-combobox" id="status">
	  						<option value=''>全部</option>
							<c:forEach items="${status}" var="map">
								<c:if test="${map.key == 8 || map.key == 9}">
								<option value='${map.key }'>${map.value }</option>
								</c:if>
							</c:forEach>
						</select>         
	...
	<div id="dd" class="easyui-dialog" title="账户存管余额" style="height:22%;width:20%"	buttons="btn"
        data-options="resizable:true,modal:true,closed:true">
    	<center>
		<div style="padding:5px;height:50">
			企业名字：<input  id="companyName" class="easyui-textbox" value="" disabled="disabled">
		</div>
		<div style="padding:5px;height:50">
			法人姓名：<input  id="customerName" class="easyui-textbox" value="" disabled="disabled">
		</div>
		<div style="padding:5px;height:50">
			存管账户可用余额：<input id="caBalance" value="" class="easyui-textbox" disabled="disabled">元
		</div>
...
<script type="text/javascript">
	...
	function searchDrProductLoan(){
 		$('#drProductLoanList').datagrid({
 			url:"../product/depositReturnLoanList.do",
 			fitColumns : true,
 			showFooter:true,
 			pagination : true,
 			pageSize:25,
	...
	function formatOper(val,row,index){  
		if((row.status == 8 ||row.status == 9 ) &&  row.loanStatus==1 && menuType=="1"){
			return	'<a href="#" class="btn l-btn l-btn-small" onclick="updateRefundDrProductLoanBtn('+index+')">回款</a>'+"   "+
            '<a href="#" class="btn l-btn l-btn-small" onclick="companyFuiouBalance('+index+')">查看企业账户余额</a>';
		}
		return '<a href="#" class="btn l-btn l-btn-small" onclick="companyFuiouBalance('+index+')">查看企业账户余额</a>';
    }
...
function companyFuiouBalance(index){
	$('#drProductLoanList').datagrid('selectRow',index);// 关键在这里 
	var row = $('#drProductLoanList').datagrid('getSelected');
	var url = "../claims/getCompanyFuiouBalance.do?id="+row.id;
	$.ajax({
		url:url,
		dataType:"json",
		success:function(data){
			if(data.flag){
				...
				$("#caBalance").textbox('setValue',data.ca_balance);
				$("#dd").dialog("open");
			}else{
