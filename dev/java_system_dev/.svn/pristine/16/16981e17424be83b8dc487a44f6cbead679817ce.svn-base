...
public class ProducTask {
    ...
    public void productRaiseEnd() {
        ...
        for (Iterator iterator = list.iterator(); iterator.hasNext(); ) {
            ...
            try {
                ...
                try {
                    SmsSendUtil.sendMsg("13120757998,15800784479",
    ...
    public void syncProductInfo() {
        ...
        String res= "";
        try {
            list= drProductInfoService.selectSyncProductInfo();
            for (Map<String ,Object> s:list) {
                dto.setProductNo(s.get("productNo").toString());
                dto.setPlatformProductName(s.get("platformProductName").toString());
                dto.setContractNo(s.get("contractNo").toString());
                dto.setRepaymentDate(s.get("repaymentDate").toString());
                dto.setInterest(s.get("interest").toString());
                dto.setLoanAmount(s.get("loanAmount").toString());
                dto.setLoanDeadline(Integer.parseInt(s.get("loanDeadline").toString()));
                dto.setLoanRate(s.get("loanRate").toString());
                dto.setLoanDate(s.get("loanDate").toString());
                dto.setEstablishDate(s.get("establishDate").toString());
                res = bypFinancialService.syncProductInfo(dto);
                log.info(res);
            }
        } catch (Exception e) {
            log.error( "==>计息任务执行失败,下一个产品开始编号为", e);
            try {
                SmsSendUtil.sendMsg("15221219118,15800784479",
                        dto.getContractNo() + "==>执行放款数据同步数据成功");
            } catch (Exception e1) {
                log.error(e1);
            }
        }
        log.info("执行放款数据同步数据成功!!");
    ...
    public void productRepay() {
        ...
        for (int i = 0; i < infoList.size(); i++) {
            ...
            try {
                ...
                try {
                    SmsSendUtil.sendMsg("15800784479,13120757998,15221219118",
    ...
    // 凌晨5点执行回款
    public void syncRepaymentStatus() {
        log.info("执行回款数据同步");
        try {
        List<DrProductInfo> infoList = drProductInvestService
                .selectExpireProductInfo();
        String[] code=new String[infoList.size()];
        for (int i = 0; i < infoList.size(); i++) {
            DrProductInfo info = infoList.get(i);
            code[i]=info.getCode();
        }
            bypFinancialService.syncRepaymentStatus(code);
        } catch (Exception e) {
            log.error( "执行回款数据同步 失败"+e.getMessage());
        }
        log.info("执行回款数据同步成功");
    }
    ...
    public void productFullRemind() {
        try {
            log.info("执行满标提醒开始...");
            String str = PropertyUtil
                    .getProperties("productFullRemind");
            if (Utils.isObjectEmpty(str))
                str = "18930352770";
            String mobliePhone[] = str.split(",");
            for (int i = 0; i < mobliePhone.length; i++) {
            	drProductInfoService.productFullRemind(mobliePhone[i]);
			}
        } catch (Exception e) {
            e.printStackTrace();
        }
